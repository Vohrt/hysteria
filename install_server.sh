...
# URLs of GitHub
REPO_URL="https://github.com/Vohrt/hysteria"
API_BASE_URL="https://api.github.com/repos/Vohrt/hysteria"
...
rerun_with_sudo() {
  ...
  if has_prefix "$0" "/dev/fd/"; then
    ...
    if has_command curl; then
      curl -o "$_tmp_script" 'https://raw.githubusercontent.com/Vohrt/hysteria/hy1/install_server.sh'
    elif has_command wget; then
      wget -O "$_tmp_script" 'https://raw.githubusercontent.com/Vohrt/hysteria/hy1/install_server.sh'
    else
      return 127
    fi
...
perform_install() {
  ...
  if [[ -n "$_is_frash_install" ]]; then
    ...
    echo
    echo -e "Check out the latest changelog $(tblue)https://github.com/Vohrt/hysteria/blob/hy1/CHANGELOG.md$(treset)"
    echo
  fi
}
